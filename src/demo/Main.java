...
public class Main {
	public static void main(String[] args) {
		...
		while (true) {
            ...
            choiceHuman(humans).attack(choiceMonster(monsters));
            // モンスターのHPが0以下になれば、モンスターは倒れ、そのモンスターをモンスターグループから削除
            if((monsters.size()==3)&&(monsters.get(2).getHp() <= 0)) {
            	monsters.remove(2);
            }else if((monsters.size()==3)&&(monsters.get(1).getHp() <= 0)) {
            	monsters.remove(1);
            }else if((monsters.size()==3)&&(monsters.get(0).getHp() <= 0)) {
            	monsters.remove(0);
            }else if((monsters.size()==2)&&(monsters.get(1).getHp() <= 0)) {
            	monsters.remove(1);
            }else if((monsters.size()==2)&&(monsters.get(0).getHp() <= 0)) {
            	monsters.remove(0);
            }else if((monsters.size()==1)&&(monsters.get(0).getHp() <= 0)) {
            	monsters.remove(0);
            }
			...
			choiceMonster(monsters).attack(choiceHuman(humans));
			// 人間のHPが0以下になれば、人間は倒れ、その人間を人間グループから削除
			if((humans.size()==3)&&(humans.get(2).getHp() <= 0)) {
            	humans.remove(2);
            }else if((humans.size()==3)&&(humans.get(1).getHp() <= 0)) {
            	humans.remove(1);
            }else if((humans.size()==3)&&(humans.get(0).getHp() <= 0)) {
            	humans.remove(0);
            }else if((humans.size()==2)&&(humans.get(1).getHp() <= 0)) {
            	humans.remove(1);
            }else if((humans.size()==2)&&(humans.get(0).getHp() <= 0)) {
            	humans.remove(0);
            }else if((humans.size()==1)&&(humans.get(0).getHp() <= 0)) {
            	humans.remove(0);
            }
